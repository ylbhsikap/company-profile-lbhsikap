      ...
      <div className="-mx-6 flex overflow-x-auto px-6 pb-6 gap-6 snap-x snap-mandatory scrollbar-none sm:mx-0 sm:grid sm:grid-cols-1 sm:gap-12 sm:px-0 sm:pb-0 sm:overflow-visible">
        {anggotaList.map((member, index) => {
          ...
          return (
            <div 
              key={member.id} 
              className="w-75 shrink-0 snap-center flex flex-col overflow-hidden rounded-xl border border-gray-200 bg-white shadow-xs sm:w-full sm:flex-none sm:grid sm:grid-cols-2"
            >
              ...
              >
                {/* 💡 Perbaikan Nama: Ditambahkan break-words, leading-tight, dan ukuran responsif agar tidak tertutup */}
                <h3 className="text-lg sm:text-xl lg:text-2xl font-black text-gray-950 uppercase tracking-tight leading-tight break-words">
                  {member.nama}
                </h3>
              ...
              {/* Kolom 2: Area Wadah Foto dengan Tinggi yang Diperbesar */}
              <div className={`relative min-h-80 w-full bg-gray-50 sm:min-h-[450px]
